...
# --- 1. アプリ設定 & デザイン ---
st.set_page_config(page_title="Owl v3.6.9", page_icon="🦉", layout="wide")
...
def fetch_url_content(url):
    ...
    try:
        ...
        if "JavaScript" in page_text and "enable" in page_text:
            return "Block", "このサイトはプログラムからのアクセスをブロックしています。内容を読み取るにはスクショを撮って『画像分析』してください。"
...
def analyze_image(client, image_file):
    ...
    res = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[{"role": "user", "content": [{"type": "text", "text": "この画像の内容を詳細に分析し、何が書かれているか、どんな雰囲気か説明してください。"}, {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{b64}"}}]}]
    )
    return res.choices[0].message.content
...
if not st.session_state['user']:
    st.markdown(f"<div class='login-box'><h1>🦉 Owl v3.6.9</h1><p>Athenalink Operation System</p></div>", unsafe_allow_html=True)
...
# --- 4. レイアウト & モジュール ---
st.sidebar.markdown(f"### 🦉 Owl v3.6.9")
...
def render_chat(mode, system_prompt):
    ...
    if submitted and user_input:
        ...
        # 画像生成トリガーチェック
        wants_image = mode == "M1 SNS" and ("画像" in user_input) and ("作って" in user_input or "生成" in user_input)
        ...
        if extracted_url:
            with st.spinner("🌍 URLを読み込んでいます..."):
                ...
                if title in ["Block", "Error"]:
                    st.error(f"URLエラー: {content}")
                else:
                    # 1. 要約生成
                    ...
                    # 5. 【ここが重要】URLの内容を踏まえたコメントを即時生成させる
                    with st.spinner("Owl is reading & thinking..."):
                        # システムプロンプトは last_knowledge で更新済みだが、
                        # 念のため直近のコンテキストとして渡す
                        temp_msgs = st.session_state[key].copy()
                        temp_msgs.append({"role": "user", "content": "このURLの内容を踏まえて、どう活用できるか、または感想を簡潔に述べてください。"})
                        
                        res = client.chat.completions.create(model="gpt-3.5-turbo", messages=temp_msgs, max_tokens=1000)
