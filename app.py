...
def generate_image(client, text_prompt):
    prompt_response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[{"role": "system", "content": "DALL-E 3用の画像生成プロンプト(英語)を作成してください。"}, {"role": "user", "content": text_prompt}]
    )
...
STYLE = "【スタイル】\n1.言語:日本語\n2.禁止:自分語り/ポエム/説教\n3.構成:受容→分析→処方\n4.態度:プロのカウンセラー"
prompts = {
    "M4": f"戦略参謀です。{STYLE} 目標達成タスクを8-15個提案して。",
    "M1": f"SNS担当です。{STYLE} 読者の心を代弁するポストを3案作成して。",
    "M2": f"編集者です。{STYLE} 記事構成(見出し5-10個)を作成して。",
    "M3": f"セールスライターです。{STYLE} PASONA(Problem/Affinity/Solution/Action)で長文レターを書いて。"
}
